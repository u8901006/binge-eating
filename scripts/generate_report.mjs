#!/usr/bin/env node
/**
 * Generate BED daily report HTML using Zhipu AI (GLM-5-Turbo).
...
const SYSTEM_PROMPT = `你是暴食症（Binge Eating Disorder, BED）領域的資深研究員與科學傳播者。你的任務是：
  ...
  try {
    ...
    if (jsonStart >= 0 && jsonEnd > jsonStart) {
      try {
        ...
        const partial = cleaned.slice(jsonStart, jsonEnd + 1);
        const fixed = partial
          .replace(/,\s*([}\]])/g, '$1')
          .replace(/"\s*\n\s*"/g, '",\n"')
          .replace(/\\(?!["\\/bfnrtu])/g, '\\\\');
...
async function analyzePapers(apiKey, papersData) {
  ...
  const prompt = `以下是 ${dateStr} 從 PubMed 抓取的最新暴食症（BED）相關文獻（共 ${paperCount} 篇）。

請進行以下分析，並以 JSON 格式回傳（不要用 markdown code block）：

{
  "date": "${dateStr}",
  "market_summary": "1-2句話總結今天文獻的整體趨勢與亮點，聚焦暴食症",
  "top_picks": [
    {
      "rank": 1,
      "title_zh": "中文標題",
...
每篇 paper 的 tags 請從以下選擇：暴食症診斷、暴食症流行病學、CBT治療、DBT治療、ACT治療、IPT治療、藥物治療、GLP-1相關、神經科學、情緒調節、創傷與PTSD、肥胖與代謝、減重手術、營養治療、體重污名、社群媒體、運動與身體活動、運動員、食物成癮、身體意象、兒少精神醫學、睡眠、文化與社會、正念飲食、直覺飲食、食品不安全。
...
<title>Binge Eating Research &middot; 暴食症文獻日報 &middot; ${dateDisplay}</title>
<meta name="description" content="${dateDisplay} 暴食症（BED）研究文獻日報，由 AI 自動彙整 PubMed 最新論文"/>
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  footer { margin-top: 32px; padding-top: 22px; border-top: 1px solid var(--line); font-size: 11.5px; color: var(--muted); display: flex; justify-content: space-between; animation: fadeUp 0.5s ease 0.5s both; flex-wrap: wrap; gap: 6px; }
...
<div class="container">
  <header>
    ...
    <div class="header-text">
      <h1>Binge Eating Research &middot; 暴食症文獻日報</h1>
  ...
  <footer>
    ...
    <span><a href="https://github.com/u8901006/binge-eating">GitHub</a></span>
  </footer>
</div>
</body>
</html>`;
...
async function main() {
  ...
  if (!papersData?.papers?.length) {
    ...
    const analysis = {
      date: tz,
      market_summary: '今日 PubMed 暫無新的暴食症相關文獻更新。請明天再查看。',
      top_picks: [],
      all_papers: [],
      keywords: [],
      topic_distribution: {},
